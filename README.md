# 📦 VespicMM — A Simple Mod Manager for Casualties: Unknown

Hey everyone! I love Casualties: Unknown and wanted to make managing mods easier for the community. This is my very first software project! I got tired of opening folders, moving files, and manually renaming things, so I built this tool:b

<img width="1536" height="1024" alt="VespicMM" src="https://github.com/user-attachments/assets/588a1d15-b85d-4775-957f-561b6ab32930" />

## ✨ Features
* **Quick BepInEx Installation:** Sets up BepInEx for you quickly.
* **One-Click Toggle:** Turn mods on or off instantly.
* **Smart DLL Grouping:** Neatly groups multiple .dll files under one name.
* **Mod Packs:** Sort your mods into separate packs easily.
* **Vanilla Launch:** Run the clean game without any mods.

## 🖼️ UI Design
<img width="844" height="626" alt="Screenshot 2026-05-25 233458" src="https://github.com/user-attachments/assets/c680c6f4-a2fd-4160-acc5-3c67fdb6cd52" />

<img width="1234" height="667" alt="Screenshot 2026-05-25 233435" src="https://github.com/user-attachments/assets/e88cd566-2cf6-487f-859e-14284243c5ca" />

## 🛠️ Status
It's almost ready! I'm just finishing up the installer and fixing some bugs. The code and the release will be available very soon! 

Дякую за увагу!:D
